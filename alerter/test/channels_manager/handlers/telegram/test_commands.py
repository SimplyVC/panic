...
class TestTelegramCommandsHandler(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.test_telegram_command_handlers = TelegramCommandHandlers(
            ...
            self.test_associated_chains, self.test_channel, self.rabbitmq,
            self.redis, self.mongo)
    ...
    def tearDown(self) -> None:
        ...
        try:
            self.test_telegram_commands_handler.rabbitmq.connect()

            # Declare them before just in case there are tests which do not
            # use these queues and exchanges
            self.test_telegram_commands_handler.rabbitmq.queue_declare(
            ...
            self.test_telegram_commands_handler.rabbitmq.queue_declare(
                ...
                False)
            self.test_telegram_commands_handler.rabbitmq.exchange_declare(
                HEALTH_CHECK_EXCHANGE, 'topic', False, True, False, False)

            self.test_telegram_commands_handler.rabbitmq.queue_purge(
            ...
            self.test_telegram_commands_handler.rabbitmq.disconnect()
